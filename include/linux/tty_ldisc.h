...
extern void __init_ldsem(struct ld_semaphore *sem, const char *name, struct lock_class_key *key);
...
#ifdef CONFIG_DEBUG_LOCK_ALLOC
extern int ldsem_down_read_nested(struct ld_semaphore *sem, int subclass, long timeout);
extern int ldsem_down_write_nested(struct ld_semaphore *sem, int subclass, long timeout);
#else
#define ldsem_down_read_nested(sem, subclass, timeout) ldsem_down_read(sem, timeout)
#define ldsem_down_write_nested(sem, subclass, timeout) ldsem_down_write(sem, timeout)
#endif
...
struct tty_ldisc_ops {
	...
	int (*compat_ioctl)(struct tty_struct *tty, struct file *file, unsigned int cmd, unsigned long arg);
	void (*set_termios)(struct tty_struct *tty, struct ktermios *old);
...
// line discipline(行规程) 充当了数据处理的中间层, 位于底部的 tty驱动 与 用户空间的应用程序之间
struct tty_ldisc {
