...
#define SYSCALL_TRACE_ENTER_EVENT(sname)                                                                                                                       \
	...
	static struct trace_event_call __used __attribute__((section("_ftrace_events"))) *__event_enter_##sname = &event_enter_##sname;
...
#define SYSCALL_TRACE_EXIT_EVENT(sname)                                                                                                                        \
	...
	static struct trace_event_call __used __attribute__((section("_ftrace_events"))) *__event_exit_##sname = &event_exit_##sname;

#define SYSCALL_METADATA(sname, nb, ...)                                                                                                                       \
	static const char *types_##sname[] = { __MAP(nb, __SC_STR_TDECL, __VA_ARGS__) };                                                                       \
	static const char *args_##sname[] = { __MAP(nb, __SC_STR_ADECL, __VA_ARGS__) };                                                                        \
...
#define __SYSCALL_DEFINEx(x, name, ...)                                                                                                                        \
	...
	asmlinkage long sys##name(__MAP(x, __SC_DECL, __VA_ARGS__)) __attribute__((alias(__stringify(__se_sys##name))));                                       \
...
static inline void addr_limit_user_check(void)
{
	...
	if (CHECK_DATA_CORRUPTION(!segment_eq(get_fs(), USER_DS), "Invalid address limit on user-mode return"))
		force_sig(SIGKILL);
...
asmlinkage long sys_io_pgetevents(aio_context_t ctx_id, long min_nr, long nr, struct io_event __user *events, struct __kernel_timespec __user *timeout,
				  const struct __aio_sigset *sig);
asmlinkage long sys_io_pgetevents_time32(aio_context_t ctx_id, long min_nr, long nr, struct io_event __user *events, struct old_timespec32 __user *timeout,
					 const struct __aio_sigset *sig);
asmlinkage long sys_io_uring_setup(u32 entries, struct io_uring_params __user *p);
asmlinkage long sys_io_uring_enter(unsigned int fd, u32 to_submit, u32 min_complete, u32 flags, const sigset_t __user *sig, size_t sigsz);
...
/* kernel/futex.c */
asmlinkage long sys_futex(u32 __user *uaddr, int op, u32 val, struct __kernel_timespec __user *utime, u32 __user *uaddr2, u32 val3);
asmlinkage long sys_futex_time32(u32 __user *uaddr, int op, u32 val, struct old_timespec32 __user *utime, u32 __user *uaddr2, u32 val3);
...
asmlinkage long sys_mq_timedreceive_time32(mqd_t mqdes, char __user *u_msg_ptr, unsigned int msg_len, unsigned int __user *u_msg_prio,
					   const struct old_timespec32 __user *u_abs_timeout);
asmlinkage long sys_mq_timedsend_time32(mqd_t mqdes, const char __user *u_msg_ptr, unsigned int msg_len, unsigned int msg_prio,
					const struct old_timespec32 __user *u_abs_timeout);
...
asmlinkage long sys_process_vm_readv(pid_t pid, const struct iovec __user *lvec, unsigned long liovcnt, const struct iovec __user *rvec, unsigned long riovcnt,
				     unsigned long flags);
asmlinkage long sys_process_vm_writev(pid_t pid, const struct iovec __user *lvec, unsigned long liovcnt, const struct iovec __user *rvec, unsigned long riovcnt,
				      unsigned long flags);
...
int ksys_mount(const char __user *dev_name, const char __user *dir_name, const char __user *type, unsigned long flags, void __user *data);
...
int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent, unsigned int count);
...
#else
static inline int ksys_fadvise64_64(int fd, loff_t offset, loff_t len, int advice)
{
	return -EINVAL;
}
#endif
...
extern long do_symlinkat(const char __user *oldname, int newdfd, const char __user *newname);

static inline long ksys_symlink(const char __user *oldname, const char __user *newname)
{
	return do_symlinkat(oldname, AT_FDCWD, newname);
}

extern long do_mknodat(int dfd, const char __user *filename, umode_t mode, unsigned int dev);

static inline long ksys_mknod(const char __user *filename, umode_t mode, unsigned int dev)
{
	return do_mknodat(AT_FDCWD, filename, mode, dev);
}

extern int do_linkat(int olddfd, const char __user *oldname, int newdfd, const char __user *newname, int flags);

static inline long ksys_link(const char __user *oldname, const char __user *newname)
{
...
static inline long ksys_open(const char __user *filename, int flags, umode_t mode)
{
...
long ksys_semtimedop(int semid, struct sembuf __user *tsops, unsigned int nsops, const struct __kernel_timespec __user *timeout);
...
long compat_ksys_semtimedop(int semid, struct sembuf __user *tsems, unsigned int nsops, const struct old_timespec32 __user *timeout);
