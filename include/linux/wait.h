...
struct wait_queue_entry {
	unsigned int flags;
	void *private; // 通常用来指向进程的 task_struct
	wait_queue_func_t func; // 进程被唤醒时, 执行的唤醒函数
...
#define __WAITQUEUE_INITIALIZER(name, tsk)                                                                                                                     \
	{                                                                                                                                                      \
		.private = tsk, .func = default_wake_function, .entry = { NULL, NULL }                                                                         \
	}
...
#define __WAIT_QUEUE_HEAD_INITIALIZER(name)                                                                                                                    \
	{                                                                                                                                                      \
		.lock = __SPIN_LOCK_UNLOCKED(name.lock), .head = { &(name).head, &(name).head }                                                                \
	}
...
static inline void __add_wait_queue_entry_tail_exclusive(struct wait_queue_head *wq_head, struct wait_queue_entry *wq_entry)
{
	wq_entry->flags |= WQ_FLAG_EXCLUSIVE;
...
#define wake_up_interruptible_sync_poll(x, m) __wake_up_sync_key((x), TASK_INTERRUPTIBLE, 1, poll_to_key(m))
...
#define __wait_event_freezable_timeout(wq_head, condition, timeout)                                                                                            \
	___wait_event(wq_head, ___wait_cond_timeout(condition), TASK_INTERRUPTIBLE, 0, timeout, __ret = freezable_schedule_timeout(__ret))
...
#define __wait_event_hrtimeout(wq_head, condition, timeout, state)                                                                                             \
	({                                                                                                                                                     \
		...
		if ((timeout) != KTIME_MAX) {                                                                                                                  \
			hrtimer_set_expires_range_ns(&__t.timer, timeout, current->timer_slack_ns);                                                            \
			hrtimer_sleeper_start_expires(&__t, HRTIMER_MODE_REL);                                                                                 \
		}                                                                                                                                              \
                                                                                                                                                               \
		__ret = ___wait_event(                                                                                                                         \
			wq_head, condition, state, 0, 0, if (!__t.task) {                                                                                      \
				__ret = -ETIME;                                                                                                                \
				break;                                                                                                                         \
			} schedule());                                                                                                                         \
                                                                                                                                                               \
...
#define wait_event_hrtimeout(wq_head, condition, timeout)                                                                                                      \
	({                                                                                                                                                     \
		...
		might_sleep();                                                                                                                                 \
		if (!(condition))                                                                                                                              \
			__ret = __wait_event_hrtimeout(wq_head, condition, timeout, TASK_UNINTERRUPTIBLE);                                                     \
		__ret;                                                                                                                                         \
	})
...
#define wait_event_interruptible_hrtimeout(wq, condition, timeout)                                                                                             \
	({                                                                                                                                                     \
		...
		might_sleep();                                                                                                                                 \
		if (!(condition))                                                                                                                              \
			__ret = __wait_event_hrtimeout(wq, condition, timeout, TASK_INTERRUPTIBLE);                                                            \
		__ret;                                                                                                                                         \
	})
...
#define __wait_event_idle_timeout(wq_head, condition, timeout)                                                                                                 \
	___wait_event(wq_head, ___wait_cond_timeout(condition), TASK_IDLE, 0, timeout, __ret = schedule_timeout(__ret))
...
#define __wait_event_idle_exclusive_timeout(wq_head, condition, timeout)                                                                                       \
	___wait_event(wq_head, ___wait_cond_timeout(condition), TASK_IDLE, 1, timeout, __ret = schedule_timeout(__ret))
...
#define __wait_event_killable_timeout(wq_head, condition, timeout)                                                                                             \
	___wait_event(wq_head, ___wait_cond_timeout(condition), TASK_KILLABLE, 0, timeout, __ret = schedule_timeout(__ret))
...
#define wait_event_killable_timeout(wq_head, condition, timeout)                                                                                               \
	({                                                                                                                                                     \
		...
		might_sleep();                                                                                                                                 \
		if (!___wait_cond_timeout(condition))                                                                                                          \
			__ret = __wait_event_killable_timeout(wq_head, condition, timeout);                                                                    \
		__ret;                                                                                                                                         \
	})

#define __wait_event_lock_irq(wq_head, condition, lock, cmd)                                                                                                   \
	(void)___wait_event(wq_head, condition, TASK_UNINTERRUPTIBLE, 0, 0, spin_unlock_irq(&lock); cmd; schedule(); spin_lock_irq(&lock))
...
#define __wait_event_interruptible_lock_irq(wq_head, condition, lock, cmd)                                                                                     \
	___wait_event(wq_head, condition, TASK_INTERRUPTIBLE, 0, 0, spin_unlock_irq(&lock); cmd; schedule(); spin_lock_irq(&lock))
...
#define wait_event_interruptible_lock_irq_cmd(wq_head, condition, lock, cmd)                                                                                   \
	({                                                                                                                                                     \
		int __ret = 0;                                                                                                                                 \
		if (!(condition))                                                                                                                              \
			__ret = __wait_event_interruptible_lock_irq(wq_head, condition, lock, cmd);                                                            \
		__ret;                                                                                                                                         \
	})
...
#define wait_event_interruptible_lock_irq(wq_head, condition, lock)                                                                                            \
	({                                                                                                                                                     \
		int __ret = 0;                                                                                                                                 \
		if (!(condition))                                                                                                                              \
			__ret = __wait_event_interruptible_lock_irq(wq_head, condition, lock, );                                                               \
		__ret;                                                                                                                                         \
	})

#define __wait_event_lock_irq_timeout(wq_head, condition, lock, timeout, state)                                                                                \
	___wait_event(wq_head, ___wait_cond_timeout(condition), state, 0, timeout, spin_unlock_irq(&lock); __ret = schedule_timeout(__ret);                    \
		      spin_lock_irq(&lock));
 ...
 */
#define wait_event_interruptible_lock_irq_timeout(wq_head, condition, lock, timeout)                                                                           \
	({                                                                                                                                                     \
		long __ret = timeout;                                                                                                                          \
		if (!___wait_cond_timeout(condition))                                                                                                          \
			__ret = __wait_event_lock_irq_timeout(wq_head, condition, lock, timeout, TASK_INTERRUPTIBLE);                                          \
		__ret;                                                                                                                                         \
	})
...
#define wait_event_lock_irq_timeout(wq_head, condition, lock, timeout)                                                                                         \
	({                                                                                                                                                     \
		long __ret = timeout;                                                                                                                          \
		if (!___wait_cond_timeout(condition))                                                                                                          \
			__ret = __wait_event_lock_irq_timeout(wq_head, condition, lock, timeout, TASK_UNINTERRUPTIBLE);                                        \
		__ret;                                                                                                                                         \
	})
