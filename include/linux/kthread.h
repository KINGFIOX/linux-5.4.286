...
#define KTHREAD_WORKER_INIT(worker)                                                                                                                            \
	{                                                                                                                                                      \
		.lock = __RAW_SPIN_LOCK_UNLOCKED((worker).lock), .work_list = LIST_HEAD_INIT((worker).work_list),                                              \
		.delayed_work_list = LIST_HEAD_INIT((worker).delayed_work_list),                                                                               \
	}

#define KTHREAD_WORK_INIT(work, fn)                                                                                                                            \
	{                                                                                                                                                      \
		.node = LIST_HEAD_INIT((work).node), .func = (fn),                                                                                             \
	}

#define KTHREAD_DELAYED_WORK_INIT(dwork, fn)                                                                                                                   \
	{                                                                                                                                                      \
		.work = KTHREAD_WORK_INIT((dwork).work, (fn)), .timer = __TIMER_INITIALIZER(kthread_delayed_work_timer_fn, TIMER_IRQSAFE),                     \
	}
...
extern void __kthread_init_worker(struct kthread_worker *worker, const char *name, struct lock_class_key *key);
...
#define kthread_init_delayed_work(dwork, fn)                                                                                                                   \
	do {                                                                                                                                                   \
		...
		timer_setup(&(dwork)->timer, kthread_delayed_work_timer_fn, 0);                                                                                \
	} while (0)
