...
static inline int tick_nohz_tick_stopped_cpu(int cpu)
{
	return 0;
}
...
extern void tick_nohz_dep_set_task(struct task_struct *tsk, enum tick_dep_bits bit);
extern void tick_nohz_dep_clear_task(struct task_struct *tsk, enum tick_dep_bits bit);
extern void tick_nohz_dep_set_signal(struct signal_struct *signal, enum tick_dep_bits bit);
extern void tick_nohz_dep_clear_signal(struct signal_struct *signal, enum tick_dep_bits bit);
...
// 无时钟滴答(nohz)模式. NO_HZ 模式旨在减少或消除在任务空闲或长时间运行单一任务时的定时器中断，以降低功耗和提高系统性能
static inline void tick_nohz_task_switch(void)
{
	if (tick_nohz_full_enabled())
		__tick_nohz_task_switch(); // defined in kernel/time/tick-sched.c
}
