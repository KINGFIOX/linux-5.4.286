 ...
 * pte bit, in which case this function is trivial(不重要). Secondly, an architecture
 * may not have a spare pte bit, which requires a more complicated scheme,
 * described below. 有两种情况: 如果 arch 有 pte_special() pte bit, 则这个函数是 trivial(不重要).
 * 如果 arch 没有 pte_special() pte bit, 则需要一个更复杂的 scheme, 描述如下.
 *
...
struct page *vm_normal_page(struct vm_area_struct *vma, unsigned long addr, pte_t pte)
{
	...
	if (IS_ENABLED(CONFIG_ARCH_HAS_PTE_SPECIAL)) {
		if (likely(!pte_special(pte))) // not set special
			goto check_pfn;
		if (vma->vm_ops && vma->vm_ops->find_special_page) // hook function valid. unlikely
			return vma->vm_ops->find_special_page(vma, addr);
		if (vma->vm_flags & (VM_PFNMAP | VM_MIXEDMAP))
			return NULL; // this is the exit of COWed page
		if (is_zero_pfn(pfn))
			return NULL;
		if (pte_devmap(pte)) // 0
			return NULL;

		print_bad_pte(vma, addr, pte, NULL); // print debug info
		return NULL;
	}
...
check_pfn:
	if (unlikely(pfn > highest_memmap_pfn)) { // bad pfn, out of range
...
static inline unsigned long copy_one_pte(struct mm_struct *dst_mm, struct mm_struct *src_mm, pte_t *dst_pte, pte_t *src_pte, struct vm_area_struct *vma,
...
static int copy_pte_range(struct mm_struct *dst_mm, struct mm_struct *src_mm, pmd_t *dst_pmd, pmd_t *src_pmd, struct vm_area_struct *vma, unsigned long addr,
			  unsigned long end)
{
...
static inline int copy_pmd_range(struct mm_struct *dst_mm, struct mm_struct *src_mm, pud_t *dst_pud, pud_t *src_pud, struct vm_area_struct *vma,
...
static inline int copy_pud_range(struct mm_struct *dst_mm, struct mm_struct *src_mm, p4d_t *dst_p4d, p4d_t *src_p4d, struct vm_area_struct *vma,
				 unsigned long addr, unsigned long end)
{
	...
	do {
		...
		if (pud_trans_huge(*src_pud) || pud_devmap(*src_pud)) {
			...
			err = copy_huge_pud(dst_mm, src_mm, dst_pud, src_pud, addr, vma);
			if (err == -ENOMEM)
				return -ENOMEM;
			if (!err)
				continue;
...
static inline int copy_p4d_range(struct mm_struct *dst_mm, struct mm_struct *src_mm, pgd_t *dst_pgd, pgd_t *src_pgd, struct vm_area_struct *vma,
				 unsigned long addr, unsigned long end)
{
	...
	do {
		...
		if (p4d_none_or_clear_bad(src_p4d))
			continue;
		if (copy_pud_range(dst_mm, src_mm, dst_p4d, src_p4d, vma, addr, next))
			return -ENOMEM;
...
int copy_page_range(struct mm_struct *dst_mm, struct mm_struct *src_mm, struct vm_area_struct *vma)
{
	...
	if (is_cow) {
		mmu_notifier_range_init(&range, MMU_NOTIFY_PROTECTION_PAGE, 0, vma, src_mm, addr, end);
	...
	do {
		...
		if (pgd_none_or_clear_bad(src_pgd))
			continue;
		if (unlikely(copy_p4d_range(dst_mm, src_mm, dst_pgd, src_pgd, vma, addr, next))) {
			ret = -ENOMEM;
			break;
		}
...
static unsigned long zap_pte_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pmd_t *pmd, unsigned long addr, unsigned long end,
...
again:
	...
	do {
		...
		if (!non_swap_entry(entry)) {
			...
			page = migration_entry_to_page(entry);
			if (details && details->check_mapping && details->check_mapping != page_rmapping(page))
				continue;
			rss[mm_counter(page)]--;
		}
...
static inline unsigned long zap_pmd_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pud_t *pud, unsigned long addr, unsigned long end,
					  struct zap_details *details)
{
	pmd_t *pmd;
	unsigned long next;
	...
	do {
		...
		if (is_swap_pmd(*pmd) || pmd_trans_huge(*pmd) || pmd_devmap(*pmd)) {
			if (next - addr != HPAGE_PMD_SIZE)
				...
				goto next;
			/* fall through */
		} else if (details && details->single_page && PageTransCompound(details->single_page) && next - addr == HPAGE_PMD_SIZE && pmd_none(*pmd)) {
...
static inline unsigned long zap_pud_range(struct mmu_gather *tlb, struct vm_area_struct *vma, p4d_t *p4d, unsigned long addr, unsigned long end,
					  struct zap_details *details)
{
	pud_t *pud;
	unsigned long next;
...
static inline unsigned long zap_p4d_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pgd_t *pgd, unsigned long addr, unsigned long end,
					  struct zap_details *details)
{
	p4d_t *p4d;
	unsigned long next;
...
static int remap_pfn_range_internal(struct vm_area_struct *vma, unsigned long addr, unsigned long pfn, unsigned long size, pgprot_t prot)
{
	pgd_t *pgd;
	unsigned long next;
...
static int apply_to_pte_range(struct mm_struct *mm, pmd_t *pmd, unsigned long addr, unsigned long end, pte_fn_t fn, void *data)
{
	...
	pte = (mm == &init_mm) ? pte_alloc_kernel(pmd, addr) : pte_alloc_map_lock(mm, pmd, addr, &ptl);
	if (!pte)
		return -ENOMEM;
...
{
	struct file *vm_file = vma->vm_file; // mmap
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, mm, vmf->address & PAGE_MASK, (vmf->address & PAGE_MASK) + PAGE_SIZE);
	mmu_notifier_invalidate_range_start(&range);
...
static vm_fault_t wp_pfn_shared(struct vm_fault *vmf)
{
	...
	if (vma->vm_ops && vma->vm_ops->pfn_mkwrite) {
		...
		if (ret & (VM_FAULT_ERROR | VM_FAULT_NOPAGE)) // error
			return ret;
 ...
 * It is done by copying the page to a new address and decrementing(消耗) the shared-page counter for the old page.
 *
 * Note that this routine assumes that the protection checks have been done by the caller
 * (the low-level page fault routine in most cases).
 * Thus we can safely just mark it writable once we've done any necessary COW.
 *
...
static vm_fault_t do_wp_page(struct vm_fault *vmf) __releases(vmf->ptl)
{
	...
	vmf->page = vm_normal_page(vma, vmf->address, vmf->orig_pte); // get page of the pfn
	if (!vmf->page) {
		/*
	...
	if (PageAnon(vmf->page)) {
		...
		if (PageKsm(vmf->page) && (PageSwapCache(vmf->page) || page_count(vmf->page) != 1))
			goto copy;
		...
		if (PageKsm(vmf->page)) {
			bool reused = reuse_ksm_page(vmf->page, vmf->vma, vmf->address);
			unlock_page(vmf->page);
			if (!reused)
				goto copy;
			wp_page_reuse(vmf);
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (mem_cgroup_try_charge_delay(page, vma->vm_mm, GFP_KERNEL, &memcg, false)) {
		ret = VM_FAULT_OOM;
		goto out_page;
	}
...
static vm_fault_t __do_fault(struct vm_fault *vmf)
{
	...
	if (unlikely(PageHWPoison(vmf->page))) {
		...
		if (ret & VM_FAULT_LOCKED) {
			if (page_mapped(page))
				unmap_mapping_pages(page_mapping(page), page->index, 1, false);
...
{
	return pmd_devmap(*pmd) /*0*/ || pmd_trans_unstable(pmd) /*0*/; // 0
}
...
vm_fault_t alloc_set_pte(struct vm_fault *vmf, struct mem_cgroup *memcg, struct page *page)
{
	...
	if (pmd_none(*vmf->pmd) && PageTransCompound(page) && IS_ENABLED(CONFIG_TRANSPARENT_HUGE_PAGECACHE)) {
		/* THP on COW? */
...
static vm_fault_t do_cow_fault(struct vm_fault *vmf)
{
	...
	if (mem_cgroup_try_charge_delay(vmf->cow_page, vma->vm_mm, GFP_KERNEL, &vmf->memcg, false)) {
 ...
 * If mmap_sem is released, vma may become invalid (for example by other thread calling munmap()).
 */
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (unlikely(pmd_none(*vmf->pmd))) {
		 ...
		 * 这个时候, 可能是 lazy allocation 的 page
		 */
		vmf->pte = NULL;
	} else {
		/* See comment in pte_alloc_one_map() */
		if (pmd_devmap_trans_unstable(vmf->pmd)) // 0
			return 0;
		/*
		 ...
		 */
		vmf->pte = pte_offset_map(vmf->pmd, vmf->address); // get the slot of pte
	...
	if (!vmf->pte) { // handle lazy allocation
		if (vma_is_anonymous(vmf->vma)) // 匿名页的处理
			return do_anonymous_page(vmf);
		else // 文件页的处理
			return do_fault(vmf);
	}

	if (!pte_present(vmf->orig_pte)) // vmf->pte != NULL && !present(vmf.orig_pte) => swap out page
		return do_swap_page(vmf);

	if (pte_protnone(vmf->orig_pte) /*0*/ && vma_is_accessible(vmf->vma)) // 0
		return do_numa_page(vmf);

	vmf->ptl = pte_lockptr(vmf->vma->vm_mm, vmf->pmd); // get mm->page_table_lock
	spin_lock(vmf->ptl);
	entry = vmf->orig_pte;
	if (unlikely(!pte_same(*vmf->pte, entry))) // vmf->pte != vmf->orig_pte. 不相同, 意味着已经被其他线程抢先修改了
		goto unlock;
	if (vmf->flags & FAULT_FLAG_WRITE) {
		if (!pte_write(entry))
			return do_wp_page(vmf); // contains: COW and write back shared page

		// write fault, but pte has write bit set, which means: CPU make software to handle
		entry = pte_mkdirty(entry); // 发生了 write fault, 但是 pte_write != 0, 有点奇怪
	}
	entry = pte_mkyoung(entry); // set accessed bit

	// set dirty bit and *vmf->pte = entry
...
static vm_fault_t __handle_mm_fault(struct vm_area_struct *vma, unsigned long address, unsigned int flags)
{
	...
	if (!vmf.pud)
		return VM_FAULT_OOM;
	if (pud_none(*vmf.pud) && __transparent_hugepage_enabled(vma)) { // *pud == NULL && enable transparent hugepage
	...
	vmf.pmd = pmd_alloc(mm, vmf.pud, address); // alloc a pmd and populate it on the pud
	if (!vmf.pmd)
		return VM_FAULT_OOM;
	if (pmd_none(*vmf.pmd) && __transparent_hugepage_enabled(vma)) {
		...
		barrier();
		if (unlikely(is_swap_pmd(orig_pmd))) { // pmd has been swap out
			VM_BUG_ON(thp_migration_supported() && !is_pmd_migration_entry(orig_pmd));
			if (is_pmd_migration_entry(orig_pmd))
				pmd_migration_entry_wait(mm, vmf.pmd);
			return 0;
		}
		// huge page or device memory mapped. do nothing
...
vm_fault_t handle_mm_fault(struct vm_area_struct *vma, unsigned long address, unsigned int flags)
{
	...
	count_memcg_event_mm(vma->vm_mm, PGFAULT); // do nothing

	/* do counter updates before entering really critical section. */
	check_sync_rss_stat(current); // do nothing
...
{
	spinlock_t *ptl;
	pmd_t *new = pmd_alloc_one(mm, address); // alloc a pmd, using buddy system
	if (!new)
		return -ENOMEM;
	...
	ptl = pud_lock(mm, pud);
#ifndef __ARCH_HAS_4LEVEL_HACK // no 4 level page table
	if (!pud_present(*pud)) {
		mm_inc_nr_pmds(mm);
		pud_populate(mm, pud, new); // populate(居住,殖民) the new pmd on the pud
	} else /* Another has populated it */
		pmd_free(mm, new);
#else
	if (!pgd_present(*pud)) {
...
int follow_invalidate_pte(struct mm_struct *mm, unsigned long address, struct mmu_notifier_range *range, pte_t **ptepp, pmd_t **pmdpp, spinlock_t **ptlp)
{
	pgd_t *pgd;
	p4d_t *p4d;
	pud_t *pud;
	...
	if (pmd_huge(*pmd)) {
		...
		if (range) {
			mmu_notifier_range_init(range, MMU_NOTIFY_CLEAR, 0, NULL, mm, address & PMD_MASK, (address & PMD_MASK) + PMD_SIZE);
			mmu_notifier_invalidate_range_start(range);
		}
	...
	if (range) {
		mmu_notifier_range_init(range, MMU_NOTIFY_CLEAR, 0, NULL, mm, address & PAGE_MASK, (address & PAGE_MASK) + PAGE_SIZE);
		mmu_notifier_invalidate_range_start(range);
	}
 ...
 * cache lines hot.
 */
static inline void process_huge_page(unsigned long addr_hint, unsigned int pages_per_huge_page, void (*process_subpage)(unsigned long addr, int idx, void *arg),
				     void *arg)
{
	int i, n, base, l;
...
long copy_huge_page_from_user(struct page *dst_page, const void __user *usr_src, unsigned int pages_per_huge_page, bool allow_pagefault)
{
	...
	for (i = 0; i < pages_per_huge_page; i++, subpage = mem_map_next(subpage, dst_page, i)) {
		if (allow_pagefault)
