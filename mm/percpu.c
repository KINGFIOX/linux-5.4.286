...
#define __pcpu_ptr_to_addr(ptr) (void __force *)((unsigned long)(ptr) + (unsigned long)pcpu_base_addr - (unsigned long)__per_cpu_start)
#endif
#else /* CONFIG_SMP */
...
#define pcpu_for_each_unpop_region(bitmap, rs, re, start, end)                                                                                                 \
	for ((rs) = (start), pcpu_next_unpop((bitmap), &(rs), &(re), (end)); (rs) < (re); (rs) = (re) + 1, pcpu_next_unpop((bitmap), &(rs), &(re), (end)))

#define pcpu_for_each_pop_region(bitmap, rs, re, start, end)                                                                                                   \
	for ((rs) = (start), pcpu_next_pop((bitmap), &(rs), &(re), (end)); (rs) < (re); (rs) = (re) + 1, pcpu_next_pop((bitmap), &(rs), &(re), (end)))
	...
	*bits = 0;
	for (block = chunk->md_blocks + i; i < pcpu_chunk_nr_blocks(chunk); block++, i++) {
		...
		*bits = ALIGN(block->contig_hint_start, align) - block->contig_hint_start;
		 ...
		 */
		if (block->contig_hint && block->contig_hint_start >= block_off && block->contig_hint >= *bits + alloc_bits) {
...
#define pcpu_for_each_md_free_region(chunk, bit_off, bits)                                                                                                     \
	for (pcpu_next_md_free_region((chunk), &(bit_off), &(bits)); (bit_off) < pcpu_chunk_map_bits((chunk));                                                 \
	     (bit_off) += (bits) + 1, pcpu_next_md_free_region((chunk), &(bit_off), &(bits)))

#define pcpu_for_each_fit_region(chunk, alloc_bits, align, bit_off, bits)                                                                                      \
	for (pcpu_next_fit_region((chunk), (alloc_bits), (align), &(bit_off), &(bits)); (bit_off) < pcpu_chunk_map_bits((chunk));                              \
	     (bit_off) += (bits), pcpu_next_fit_region((chunk), (alloc_bits), (align), &(bit_off), &(bits)))
...
static void pcpu_block_update(struct pcpu_block_md *block, int start, int end)
{
	...
	if (contig > block->contig_hint) {
		...
		if (block->contig_hint_start && (!start || __ffs(start) > __ffs(block->contig_hint_start))) {
			/* start has a better alignment so use it */
			block->contig_hint_start = start;
			if (start < block->scan_hint_start && block->contig_hint > block->scan_hint)
				block->scan_hint = 0;
...
static void pcpu_block_refresh_hint(struct pcpu_chunk *chunk, int index)
{
	...
	pcpu_for_each_unpop_region(alloc_map, rs, re, start, PCPU_BITMAP_BLOCK_BITS)
	{
...
static void pcpu_block_update_hint_alloc(struct pcpu_chunk *chunk, int bit_off, int bits)
{
	...
	if (s_off == s_block->first_free)
		s_block->first_free = find_next_zero_bit(pcpu_index_alloc_map(chunk, s_index), PCPU_BITMAP_BLOCK_BITS, s_off + bits);

	if (pcpu_region_overlap(s_block->scan_hint_start, s_block->scan_hint_start + s_block->scan_hint, s_off, s_off + bits))
		s_block->scan_hint = 0;

	if (pcpu_region_overlap(s_block->contig_hint_start, s_block->contig_hint_start + s_block->contig_hint, s_off, s_off + bits)) {
	...
	if (s_index != e_index) {
		 ...
		 */
		e_block->first_free = find_next_zero_bit(pcpu_index_alloc_map(chunk, e_index), PCPU_BITMAP_BLOCK_BITS, e_off);
	...
	if (pcpu_region_overlap(chunk_md->scan_hint_start, chunk_md->scan_hint_start + chunk_md->scan_hint, bit_off, bit_off + bits))
		chunk_md->scan_hint = 0;
 ...
 */
static bool pcpu_is_populated(struct pcpu_chunk *chunk, int bit_off, int bits, int *next_off)
{
 ...
 */
static int pcpu_find_block_fit(struct pcpu_chunk *chunk, int alloc_bits, size_t align, bool pop_only)
{
	struct pcpu_block_md *chunk_md = &chunk->chunk_md;
	 ...
	 */
	bit_off = ALIGN(chunk_md->contig_hint_start, align) - chunk_md->contig_hint_start;
 ...
 */
static unsigned long pcpu_find_zero_area(unsigned long *map, unsigned long size, unsigned long start, unsigned long nr, unsigned long align_mask,
					 unsigned long *largest_off, unsigned long *largest_bits)
{
...
static int pcpu_alloc_area(struct pcpu_chunk *chunk, int alloc_bits, size_t align, int start)
{
	 ...
	 */
	end = min_t(int, start + alloc_bits + PCPU_BITMAP_BLOCK_BITS, pcpu_chunk_map_bits(chunk));
	bit_off = pcpu_find_zero_area(chunk->alloc_map, end, start, alloc_bits, align_mask, &area_off, &area_bits);
	if (bit_off >= end)
		return -1;
	...
	if (bit_off == chunk_md->first_free)
		chunk_md->first_free = find_next_zero_bit(chunk->alloc_map, pcpu_chunk_map_bits(chunk), bit_off + alloc_bits);
...
static struct pcpu_chunk *__init pcpu_alloc_first_chunk(unsigned long tmp_addr, int map_size)
{
	...
	/* allocate chunk */
	alloc_size = sizeof(struct pcpu_chunk) + BITS_TO_LONGS(region_size >> PAGE_SHIFT) * sizeof(unsigned long);
	...
	if (chunk->end_offset) {
		...
		bitmap_set(chunk->alloc_map, pcpu_chunk_map_bits(chunk) - offset_bits, offset_bits);
		set_bit((start_offset + map_size) / PCPU_MIN_ALLOC_SIZE, chunk->bound_map);
		set_bit(region_bits, chunk->bound_map);

		pcpu_block_update_hint_alloc(chunk, pcpu_chunk_map_bits(chunk) - offset_bits, offset_bits);
	}
...
static struct pcpu_chunk *pcpu_alloc_chunk(gfp_t gfp)
{
	...
	chunk->alloc_map = pcpu_mem_zalloc(BITS_TO_LONGS(region_bits) * sizeof(chunk->alloc_map[0]), gfp);
	if (!chunk->alloc_map)
		goto alloc_map_fail;

	chunk->bound_map = pcpu_mem_zalloc(BITS_TO_LONGS(region_bits + 1) * sizeof(chunk->bound_map[0]), gfp);
	if (!chunk->bound_map)
		goto bound_map_fail;

	chunk->md_blocks = pcpu_mem_zalloc(pcpu_chunk_nr_blocks(chunk) * sizeof(chunk->md_blocks[0]), gfp);
	if (!chunk->md_blocks)
		goto md_blocks_fail;
 ...
 */
static void pcpu_chunk_depopulated(struct pcpu_chunk *chunk, int page_start, int page_end)
{
	int nr = page_end - page_start;
...
static void pcpu_depopulate_chunk(struct pcpu_chunk *chunk, int page_start, int page_end);
...
area_found:
	...
	if (!is_atomic) {
		...
		pcpu_for_each_unpop_region(chunk->populated, rs, re, page_start, page_end)
		{
	...
	trace_percpu_alloc_percpu(reserved, is_atomic, size, align, chunk->base_addr, off, ptr);
...
static void pcpu_balance_workfn(struct work_struct *work)
{
	...
	list_for_each_entry_safe (chunk, next, &to_free, list) {
		...
		pcpu_for_each_pop_region(chunk->populated, rs, re, 0, chunk->nr_pages)
		{
...
retry_pop:
	if (pcpu_atomic_alloc_failed) {
		...
		nr_to_pop = clamp(PCPU_EMPTY_POP_PAGES_HIGH - pcpu_nr_empty_pop_pages, 0, PCPU_EMPTY_POP_PAGES_HIGH);
	}
	...
	for (slot = pcpu_size_to_slot(PAGE_SIZE); slot < pcpu_nr_slots; slot++) {
		...
		pcpu_for_each_unpop_region(chunk->populated, rs, re, 0, chunk->nr_pages)
		{
	...
	for_each_possible_cpu (cpu) {
		...
		if (va >= start && va < start + static_size) {
			if (can_addr) {
				...
				*can_addr += (unsigned long)per_cpu_ptr(base, get_boot_cpu_id());
			}
			return true;
		}
	}
...
phys_addr_t per_cpu_ptr_to_phys(void *addr)
{
	 ...
	 */
	first_low = (unsigned long)pcpu_base_addr + pcpu_unit_page_offset(pcpu_low_unit_cpu, 0);
	first_high = (unsigned long)pcpu_base_addr + pcpu_unit_page_offset(pcpu_high_unit_cpu, pcpu_unit_pages);
	if ((unsigned long)addr >= first_low && (unsigned long)addr < first_high) {
		for_each_possible_cpu (cpu) {
			void *start = per_cpu_ptr(base, cpu);
...
struct pcpu_alloc_info *__init pcpu_alloc_alloc_info(int nr_groups, int nr_units)
{
	...
	base_size = ALIGN(struct_size(ai, groups, nr_groups), __alignof__(ai->groups[0].cpu_map[0]));
 ...
 */
static void pcpu_dump_alloc_info(const char *lvl, const struct pcpu_alloc_info *ai)
{
	...
	printk("%spcpu-alloc: s%zu r%zu d%zu u%zu alloc=%zu*%zu", lvl, ai->static_size, ai->reserved_size, ai->dyn_size, ai->unit_size,
	       ai->alloc_size / ai->atom_size, ai->atom_size);
	...
	for (group = 0; group < ai->nr_groups; group++) {
		...
		for (alloc_end += gi->nr_units / upa; alloc < alloc_end; alloc++) {
			if (!(alloc % apl)) {
				pr_cont("\n");
...
#define PCPU_SETUP_BUG_ON(cond)                                                                                                                                \
	...
	PCPU_SETUP_BUG_ON(!(IS_ALIGNED(PCPU_BITMAP_BLOCK_SIZE, PAGE_SIZE) || IS_ALIGNED(PAGE_SIZE, PCPU_BITMAP_BLOCK_SIZE)));
...
#undef PCPU_SETUP_BUG_ON
	...
	pcpu_slot = memblock_alloc(pcpu_nr_slots * sizeof(pcpu_slot[0]), SMP_CACHE_BYTES);
	if (!pcpu_slot)
		panic("%s: Failed to allocate %zu bytes\n", __func__, pcpu_nr_slots * sizeof(pcpu_slot[0]));
 ...
 */
static struct pcpu_alloc_info *__init pcpu_build_alloc_info(size_t reserved_size, size_t dyn_size, size_t atom_size, pcpu_fc_cpu_distance_fn_t cpu_distance_fn)
{
	...
	next_group:
		for_each_possible_cpu (tcpu) {
			...
			if (group_map[tcpu] == group && cpu_distance_fn &&
			    (cpu_distance_fn(cpu, tcpu) > LOCAL_DISTANCE || cpu_distance_fn(tcpu, cpu) > LOCAL_DISTANCE)) {
				group++;
 ...
 */
int __init pcpu_embed_first_chunk(size_t reserved_size, size_t dyn_size, size_t atom_size, pcpu_fc_cpu_distance_fn_t cpu_distance_fn,
				  pcpu_fc_alloc_fn_t alloc_fn, pcpu_fc_free_fn_t free_fn)
{
