...
static __attribute__((optimize("O0"))) const struct fdt_property *fdt_get_property_namelen_(const void *fdt, //
											    int offset, //
											    const char *name, //
											    int namelen, //
											    int *lenp, //
...
__attribute__((optimize("O0"))) const void *fdt_getprop_namelen(const void *fdt, int nodeoffset, const char *name, int namelen, int *lenp)
{
	int poffset;
	const struct fdt_property *prop;
...
__attribute__((optimize("O0"))) const void *fdt_getprop(const void *fdt, int nodeoffset, const char *name, int *lenp)
{
...
int fdt_get_path(const void *fdt, int nodeoffset, char *buf, int buflen)
{
	...
	for (offset = 0, depth = 0; (offset >= 0) && (offset <= nodeoffset); offset = fdt_next_node(fdt, offset, &depth)) {
		while (pdepth > depth) {
			do {
				p--;
			} while (buf[p - 1] != '/');
