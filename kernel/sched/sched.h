...
{
	return dl_b->bw != -1 && dl_b->bw * cpus < dl_b->total_bw - old_bw + new_bw;
}
...
/*
 * This is the main, per-CPU runqueue data structure. 包含了调度相关的各种信息
 *
 * Locking rule: those places that want to lock multiple runqueues
 * (such as the load balancing or the thread migration code),
 * lock acquire operations must be ordered by ascending &runqueue. (这是一个典型的哲学家吃饭问题, 同时要获取两把锁)
 */
struct rq {
	/* runqueue lock: */
...
static inline void task_rq_unlock(struct rq *rq, struct task_struct *p, struct rq_flags *rf) __releases(rq->lock) __releases(p->pi_lock)
{
	rq_unpin_lock(rq, rf);
	raw_spin_unlock(&rq->lock);
...
#define for_class_range(class, _from, _to) for (class = (_from); class != (_to); class = class->next)

#define for_each_class(class) for_class_range(class, sched_class_highest, NULL)
 ...
 */
static inline int _double_lock_balance(struct rq *this_rq, struct rq *busiest) __releases(this_rq->lock) __acquires(busiest->lock) __acquires(this_rq->lock)
{
...
extern void print_numa_stats(struct seq_file *m, int node, unsigned long tsf, unsigned long tpf, unsigned long gsf, unsigned long gpf);
#endif /* CONFIG_NUMA_BALANCING */
 ...
 */
static __always_inline unsigned int uclamp_util_with(struct rq *rq, unsigned int util, struct task_struct *p)
{
...
static inline unsigned int uclamp_util_with(struct rq *rq, unsigned int util, struct task_struct *p)
{
	return util;
}
static inline unsigned int uclamp_util(struct rq *rq, unsigned int util)
...
static inline unsigned long schedutil_cpu_util(int cpu, unsigned long util_cfs, unsigned long max, enum schedutil_type type, struct task_struct *p)
{
	return 0;
}
