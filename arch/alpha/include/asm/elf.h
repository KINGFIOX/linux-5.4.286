...
struct thread_info;
struct task_struct;
extern void dump_elf_thread(elf_greg_t *dest, struct pt_regs *pt, struct thread_info *ti);
...
#define ELF_PLATFORM                                                                                                                                           \
	({                                                                                                                                                     \
		...
		(i_ == IMPLVER_EV4 ? "ev4" : i_ == IMPLVER_EV5 ? (amask(AMASK_BWX) ? "ev5" : "ev56") : amask(AMASK_CIX) ? "ev6" : "ev67");                     \
	})

#define SET_PERSONALITY(EX) set_personality(((EX).e_flags & EF_ALPHA_32BIT) ? PER_LINUX_32BIT : PER_LINUX)
