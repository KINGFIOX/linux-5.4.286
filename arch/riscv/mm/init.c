...
void __init setup_bootmem(void)
{
	...
	for_each_memblock (memory, reg) {
		...
		if (reg->base <= vmlinux_start && vmlinux_end <= end) {
			...
			if (reg->base + mem_size < end)
				memblock_remove(reg->base + mem_size, end - reg->base - mem_size);
		}
	}
	BUG_ON(mem_size == 0);
	...
	for_each_memblock (memory, reg) {
		...
		memblock_set_node(PFN_PHYS(start_pfn), PFN_PHYS(end_pfn - start_pfn), &memblock.memory, 0);
	}
}

unsigned long va_pa_offset; // 0xffff_ffdf_7fe0_0000
...
#define create_pgd_next_mapping(__nextp, __va, __pa, __sz, __prot) create_pmd_mapping(__nextp, __va, __pa, __sz, __prot)
...
#define create_pgd_next_mapping(__nextp, __va, __pa, __sz, __prot) create_pte_mapping(__nextp, __va, __pa, __sz, __prot)
#define PTE_PARENT_SIZE PGDIR_SIZE
#define fixmap_pgd_next fixmap_pte
#endif

static void __init create_pgd_mapping(pgd_t *pgdp, uintptr_t va, phys_addr_t pa, phys_addr_t sz, pgprot_t prot)
{
	pgd_next_t *nextp;
...
static uintptr_t __init best_map_size(phys_addr_t base, phys_addr_t size)
{
	...
	if (!(base & (PTE_PARENT_SIZE - 1)) && !(size & (PTE_PARENT_SIZE - 1)))
...
asmlinkage void __init setup_vm(uintptr_t dtb_pa)
{
	...
	create_pgd_mapping(early_pg_dir, FIXADDR_START, (uintptr_t)fixmap_pgd_next, PGDIR_SIZE, PAGE_TABLE);

#ifndef __PAGETABLE_PMD_FOLDED
	/* Setup fixmap PMD */
	create_pmd_mapping(fixmap_pmd, FIXADDR_START, (uintptr_t)fixmap_pte, PMD_SIZE, PAGE_TABLE);
	/* Setup trampoline PGD and PMD */
	create_pgd_mapping(trampoline_pg_dir, PAGE_OFFSET, (uintptr_t)trampoline_pmd, PGDIR_SIZE, PAGE_TABLE);
	create_pmd_mapping(trampoline_pmd, PAGE_OFFSET, load_pa, PMD_SIZE, PAGE_KERNEL_EXEC);
#else
	/* Setup trampoline PGD */
	create_pgd_mapping(trampoline_pg_dir, PAGE_OFFSET, load_pa, PGDIR_SIZE, PAGE_KERNEL_EXEC);
#endif
	...
	for (va = PAGE_OFFSET; va < end_va; va += map_size)
		create_pgd_mapping(early_pg_dir, va, load_pa + (va - PAGE_OFFSET), map_size, PAGE_KERNEL_EXEC);

	/* Create fixed mapping for early FDT parsing */
	end_va = __fix_to_virt(FIX_FDT) + FIX_FDT_SIZE;
	for (va = __fix_to_virt(FIX_FDT); va < end_va; va += PAGE_SIZE)
		create_pte_mapping(fixmap_pte, va, dtb_pa + (va - __fix_to_virt(FIX_FDT)), PAGE_SIZE, PAGE_KERNEL);
...
static void __init setup_vm_final(void)
{
	...
	for_each_memblock (memory, reg /*iterator*/) {
		...
		for (pa = start; pa < end; pa += map_size) {
			...
			create_pgd_mapping(swapper_pg_dir, va, pa, map_size, PAGE_KERNEL_EXEC);
		}
	}
